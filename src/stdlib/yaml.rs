...
impl YamlValue {
    ...
    fn as_bool(self: &YamlValue) -> Option<bool> {
        match self {
            YamlValue::Bool(b) => return Option::Some(*b),
    ...
    fn as_number(self: &YamlValue) -> Option<f64> {
        match self {
            YamlValue::Number(n) => return Option::Some(*n),
            _ => return Option::None,
        }
    }
    ...
    fn as_string(self: &YamlValue) -> Option<String> {
        match self {
            YamlValue::String(s) => return Option::Some(*s),
            _ => return Option::None,
        }
    }
    ...
    fn as_sequence(self: &YamlValue) -> Option<Vec<YamlValue> > {
        match self {
            YamlValue::Sequence(seq) => return Option::Some(*seq),
            _ => return Option::None,
        }
    }
    ...
    fn as_mapping(self: &YamlValue) -> Option<HashMap<String, YamlValue> > {
        match self {
            YamlValue::Mapping(map) => return Option::Some(*map),
            _ => return Option::None,
        }
    }
        ...
        match self {
            YamlValue::Sequence(seq) => {
                if index >= 0 && (index as usize) < seq.len() {
                    return Option::Some(seq[index as usize]);
                } else {
                    return Option::None;
                }
            }
...
impl YamlParser {
    ...
    fn parse_scalar(self: &mut YamlParser) -> Result<YamlValue, String> {
        ...
        // Read until newline, comma, colon, or end of input
        loop {
            let ch = self.current_char();
            if ch.is_empty() || ch == "\n" || ch == "," || ch == "]" || ch == "}" || ch == ":" {
                break;
        ...
        let num = value.parse_float();
        // In JavaScript, NaN !== NaN, so we check if num equals itself
        if num == num {
...
mod tests {
    ...
    fn test_yaml_definition_contains_parser() {
        ...
        assert!(YAML_DEFINITION.contains("fn yaml_parse("));
        assert!(YAML_DEFINITION.contains("fn yaml_stringify("));
    }
